...
Our equivalent is `python cli.py --service health`, which validates that modules load and configuration exists. It does not configure anything. The gap between "validate what exists" and "guide you through setup" is the difference between a developer tool and a product.
...
| **Onboarding experience** | `openclaw onboard --install-daemon` — guided wizard, auto-generates secrets, installs daemon | `python cli.py --service health` — validates only, does not configure |
