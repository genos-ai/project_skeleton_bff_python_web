...
Each document in this set is self-contained enough to be read independently, but they form a coherent whole. Core Principles (02) define the non-negotiable mandates. Backend Architecture (04) and Module Structure (05) define how code is organized. Concurrency and Resilience (30) defines the concurrency model, parallelism patterns, and resilience standards (circuit breakers, retries, timeouts) that apply to all external calls. Coding Standards (10, 11), Testing (30), and Workflow (21) define how code is written and shipped. Security (21), Data Protection (22), and Authentication (21) define how it is secured. Observability (30) defines the three pillars: logs, metrics, and distributed traces. Deployment (21, 22) defines how it runs. Optional modules — Data Layer (20), Events (21), Frontend (22), LLM (30), Telegram (20, 23), Agentic AI (25, 26), Agent-First Infrastructure (33), TUI (32), Multi-Channel Gateway (33) — extend the core when projects need those capabilities.
...
Doc 30 defines the shared resilience patterns (circuit breaker, retry, bulkhead, timeout) and concurrency model (asyncio, thread pools, process pools) that all service-layer and consumer code uses. Doc 30 defines the three-pillar observability standard (logs, metrics, traces) that all components emit into.
...
If adopting 27-opt-multi-channel-gateway.md, ensure 04-core-backend-architecture.md and 24-opt-telegram-bot-integration.md are in place. Doc 33 benefits from 25/26 for agent routing but can operate with any backend handler.
