...
- 2.0.0 (2026-03-01): Added FastStream as standard event framework. Added consumer resilience patterns (circuit breaker, retry, timeout — references 24-concurrency-and-resilience.md). Added concrete dead letter queue implementation. Added consumer lag monitoring with Prometheus metrics. Added backpressure patterns. Added trace context propagation in event envelope. Added Faust deprecation notice. Added broker selection guide for upgrade path. Integrated with 12-observability.md resilience event logging.
...
This version also addresses three gaps that matter for production systems: **consumer resilience** (what happens when a consumer's downstream dependency fails), **backpressure** (what happens when producers outpace consumers), and **observability** (how to detect and alert on consumer lag before it becomes a business problem). The resilience patterns follow **24-concurrency-and-resilience.md** — the same circuit breaker → retry → timeout layering that applies to HTTP calls applies to event consumers.

The transactional outbox pattern is unchanged — it remains the correct solution for the dual-write problem. This module provides the communication backbone for inter-module events (04), real-time client updates (07, 29), and is a required dependency for the agentic architecture (25).
...
- Real-time updates to clients (via 29-multi-channel-gateway.md)
- Audit trail requirements
- Streaming data processing (market data, telemetry)
- Agent lifecycle notifications (25-agentic-architecture.md)
...
- **AsyncIO-native** — matches FastAPI and the concurrency model in 24-concurrency-and-resilience.md.
...
Rationale:
- Already deployed for caching and task queues (doc 19)
...
Schedule this as a Taskiq periodic task (doc 19) running every 5 minutes for each consumer group.
...
All consumer resilience follows the patterns in **24-concurrency-and-resilience.md**. All resilience events are logged per the contract in **12-observability.md**.
...
_notification_breaker = aiobreaker.CircuitBreaker(
    fail_max=5,
    timeout_duration=30,
    listeners=[ResilienceLogger("notification_service")],  # From doc 12
)
...
All Redis Streams must have a `MAXLEN` cap. Unbounded streams are forbidden (per P5 — O3 Bounded Resource Usage in doc 01).
...
| trace_id | OpenTelemetry trace ID for distributed trace correlation (see doc 12). Null if tracing disabled. |
| payload | Event-specific data |

### Trace Context in Events

When OpenTelemetry tracing is enabled (doc 12), include `trace_id` in the event envelope. This links the event to the distributed trace that produced it, enabling end-to-end visibility from HTTP request → event publish → consumer processing:
...
For real-time client updates, see **29-multi-channel-gateway.md** which provides the comprehensive WebSocket connection management, session routing, and multi-channel push. The patterns below apply when 29 is not adopted.
...
- Agent autonomous decisions (doc 25)
...
|----------|-------------|
| 01-core-principles.md | O3 (Bounded Resource Usage) — MAXLEN caps, backpressure. P6 (Idempotency) — exactly-once processing. |
| 04-module-structure.md | Events are the inter-module communication mechanism |
| 07-frontend-architecture.md | WebSocket real-time updates consume events |
| 12-observability.md | Resilience event logging contract, trace context correlation, consumer lag metrics |
| 19-background-tasks.md | Outbox relay and lag collection run as scheduled tasks |
| 24-concurrency-and-resilience.md | Circuit breaker, retry, timeout patterns for consumer resilience |
| 25-agentic-architecture.md | Agent lifecycle events published through this module |
| 26-agentic-pydanticai.md | Agent execution events published through this module |
| 29-multi-channel-gateway.md | Gateway pushes events to connected clients via WebSocket |
