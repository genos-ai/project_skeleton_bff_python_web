...
| `ThreadPoolExecutor.submit()` | ❌ Lost | ❌ Lost | Use `TracedThreadPoolExecutor` (doc 30) |
...
All `ThreadPoolExecutor` usage must use the `TracedThreadPoolExecutor` wrapper from doc 30. This propagates both structlog context and OpenTelemetry spans:
...
For Taskiq tasks (doc 23), pass `request_id` and `correlation_id` as task arguments:
...
For `ProcessPoolExecutor` (doc 30), propagate trace context explicitly:
...
- Returns 503 if any configured dependency is unhealthy or if the service is shutting down (see doc 30 graceful shutdown)
...
- Token storage: Redis with TTL (reuses existing Redis from doc 23)
...
- Authentication: Requires admin or developer role (doc 21)
...
- **Open file descriptors** (critical on macOS — default limit 256, see doc 30)
