...
- 3.0.0 (2026-03-01): Added three-pillar observability (logs, metrics, traces). Added OpenTelemetry distributed tracing as core standard. Added context propagation across async/thread/process boundaries. Added profiling tools (py-spy, yappi, Scalene, asyncio pstree). Added resilience event logging contract. Added circuit breaker and concurrency metrics. Added multi-process Prometheus guidance. Promoted Telemetry Debug API from future enhancement to standard. Integrated with 24-concurrency-and-resilience.md.
...
For the full concurrency model and technical details, see **24-concurrency-and-resilience.md**. This section defines the observability-specific requirements.
...
| `ThreadPoolExecutor.submit()` | ❌ Lost | ❌ Lost | Use `TracedThreadPoolExecutor` (doc 24) |
...
| Redis Streams consumer | ❌ Lost | ❌ Lost | Include in event envelope `correlation_id` field (doc 06) |

### TracedThreadPoolExecutor

All `ThreadPoolExecutor` usage must use the `TracedThreadPoolExecutor` wrapper from doc 24. This propagates both structlog context and OpenTelemetry spans:
...
For Taskiq tasks (doc 19), pass `request_id` and `correlation_id` as task arguments:
...
For Redis Streams consumers (doc 06), the `correlation_id` in the event envelope is the link:
...
For `ProcessPoolExecutor` (doc 24), propagate trace context explicitly:
...
- Returns 503 if any configured dependency is unhealthy or if the service is shutting down (see doc 24 graceful shutdown)
...
When running multiple Uvicorn workers (docs 21, 22), each worker is a separate process with its own metric registry. Without coordination, metrics from different workers conflict and produce incorrect aggregates.
...
- Token storage: Redis with TTL (reuses existing Redis from doc 19)
...
- Authentication: Requires admin or developer role (doc 09)
...
- **Open file descriptors** (critical on macOS — default limit 256, see doc 24)
...
For Azure-hosted services (doc 22), use Azure Monitor as an alternative backend:
...
|----------|-------------|
| 01-core-principles.md | O1 (Observable by Default) — this doc is the implementation |
| 03-backend-architecture.md | Request context middleware, health check integration |
| 06-event-architecture.md | `correlation_id` in event envelope links to request traces |
| 08-llm-integration.md | LLM-specific circuit breaker and cost metrics reference this doc's resilience logging contract |
| 09-authentication.md | Telemetry Debug API requires admin/developer role |
| 17-security-standards.md | Security event logging feeds into audit trail |
| 19-background-tasks.md | Context propagation for Taskiq workers |
| 21-deployment-bare-metal.md | Production stack deployment (Prometheus, Loki, Grafana, Tempo) |
| 22-deployment-azure.md | Azure Monitor exporter configuration |
| 24-concurrency-and-resilience.md | `TracedThreadPoolExecutor`, process context propagation, resilience patterns, profiling tools |
| 25-agentic-architecture.md | Agent task tracing, cost tracking observability |
| 26-agentic-pydanticai.md | Agent execution tracing, structlog.contextvars patterns |
