...
The single-scheduler-instance rule (only one scheduler process runs, ever) prevents the duplicate execution problem that plagues cron-based systems. This document was extracted from backend architecture (04) into its own standard because background task patterns touch deployment (21, 22), observability (30), and integration modules like Telegram (24) — enough cross-cutting concerns to justify dedicated guidance.
...
| 31-ai-agentic-architecture.md | Agent plan execution dispatched as background tasks |
