...
The application code is identical regardless of whether it deploys to bare metal or Azure (22). This document covers only the infrastructure: server setup, service configuration, release management, backup procedures, and monitoring integration. It implements the security standards (21) at the infrastructure level and integrates with background tasks (23) for worker and scheduler service definitions.
...
|---------|-------|-----------|
| `--loop uvloop` | uvloop event loop | 2–4x faster than default (doc 30) |
| `--timeout-graceful-shutdown 30` | 30 seconds to drain | Matches shutdown sequence in doc 30 |
...
Only one scheduler instance should run (see doc 23):
...
OpenTelemetry is configured in the application via `config/settings/observability.yaml` (see doc 30). The infrastructure components above are deployed as systemd services alongside the application.
