...
File size limits are tighter than the backend (300 lines for components, 150 for hooks) because large React components are almost always doing too many things. Tailwind CSS is the only approved styling approach — no CSS modules, no styled-components, no inline styles — because a single styling system eliminates the "which approach do I use?" question and keeps AI code assistance consistent. This module complements the Python coding standards (07) for backend development.
