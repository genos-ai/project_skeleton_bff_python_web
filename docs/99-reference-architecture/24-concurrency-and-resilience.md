# 24 - Concurrency, Parallelism, and Resilience
...
The reference architecture standardized background tasks (19) and event-driven communication (06) but left a gap: what happens *inside* a single process when it needs to do multiple things at once, handle CPU-bound work, survive external failures, and maintain observability across all of it.

This gap matters because the architecture's primary workloads — trading signal processing, LLM orchestration (25, 26), multi-channel delivery (29), and real-time data pipelines — are all concurrency-intensive. A trading signal that takes 200ms because API calls run sequentially instead of in parallel is a trading signal that arrives too late. An LLM call that retries without backoff or a circuit breaker hammers a failing provider and cascades the failure to every other request in the process.

The existing standards touch concurrency in fragments: doc 03 mentions `asyncio.TaskGroup` briefly, doc 08 defines a circuit breaker for LLM calls only, doc 12 standardizes structlog but doesn't address context propagation across threads or processes, and doc 19 handles out-of-process task queues. This document consolidates concurrency into a single prescriptive standard, extracts resilience from its LLM-specific silo into a cross-cutting pattern, and addresses the Python 3.14 upgrade that unlocks the next generation of concurrency primitives.
...
3. Update deployment configs: doc 21 systemd units, doc 22 Azure `pythonVersion` and `runtimeStack`
...
`asyncio.TaskGroup` is the **only** pattern for parallel async operations where all must succeed. This supersedes the brief mention in doc 03.
...
_market_data_semaphore = asyncio.Semaphore(20)   # Max 20 concurrent calls
...
Resilience is not an LLM-specific concern (doc 08). Every external call — market data APIs, broker APIs, payment providers, databases, Redis, third-party services — needs protection. The standard pattern layers defences from outside in:
...
- **Never retry non-idempotent operations** unless the operation uses idempotency keys (per P6 in doc 01).
...
When adding OpenTelemetry (see doc 12 production stack), propagate trace context across process boundaries:
...
For background tasks (doc 19), pass `request_id` and `correlation_id` as task arguments:
...
All concurrency and resilience settings are centralized in YAML configuration per doc 03:
...
- [ ] Upgrade to Python 3.14 (update `pyproject.toml`, deployment configs in docs 21/22)
...
- [ ] Update doc 12 health checks to include circuit breaker state reporting
...
|----------|-------------|
| 01-core-principles.md | P5 (Fail Fast), P6 (Idempotency), O2 (Graceful Degradation), O3 (Bounded Resources) — this doc implements all four |
| 03-backend-architecture.md | Extends async patterns section; supersedes TaskGroup and timeout guidance |
| 06-event-architecture.md | Resilience patterns apply to event consumers and publishers |
| 08-llm-integration.md | Circuit breaker and retry patterns extracted to this doc as cross-cutting; doc 08 references this doc |
| 12-observability.md | Context propagation patterns extend structlog guidance; profiling tools complement production monitoring |
| 19-background-tasks.md | Context propagation applies to Taskiq workers |
| 21-deployment-bare-metal.md | Python 3.14 upgrade, uvloop, tini, graceful shutdown |
| 22-deployment-azure.md | Python 3.14 upgrade, uvloop, graceful shutdown, App Service configuration |
| 25-agentic-architecture.md | Timeout and resilience patterns apply to agent orchestration |
| 26-agentic-pydanticai.md | Context propagation applies to agent task execution |
