...
The critical design decision is that the bot is a thin presentation layer, identical in role to the web frontend or CLI. All business logic lives in the backend service layer (04). Bot handlers call backend services; they never access the database directly. This ensures that bot-specific code handles only Telegram-specific concerns (keyboards, FSM states, message formatting) while business rules remain centralized and testable. For data acquisition capabilities (channel scraping, message history), see the Client API integration (25).
...
- [Telegram Client Integration](25-opt-telegram-client-integration.md) - Client API (MTProto) for channel scraping and history
- [Background Tasks](15-core-background-tasks.md) - Taskiq integration
- [Observability](08-core-observability.md) - Logging and monitoring
- [Authentication](06-core-authentication.md) - Security patterns
