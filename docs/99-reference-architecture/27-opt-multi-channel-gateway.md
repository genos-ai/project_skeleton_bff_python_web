...
**Dependencies**: This module requires **04-core-backend-architecture.md** (FastAPI application), **24-opt-telegram-bot-integration.md** (reference channel adapter), and benefits from **31-ai-agentic-architecture.md** + **32-ai-agentic-pydanticai.md** for agent routing.
...
The existing architecture defines several client types — React web frontend (22), CLI (22), Telegram bot (24), TUI (32) — each connecting to the FastAPI backend independently. Each has its own connection model, authentication mechanism, and message format. There is no shared session layer across these clients, no centralized security enforcement point, and no mechanism for the backend to push events to clients that are not actively polling.
...
The FastAPI application exposes a WebSocket endpoint for real-time bidirectional communication. This serves the TUI (doc 32), the React frontend (22), and any other client that needs server-pushed events.
...
- [31-ai-agentic-architecture.md](31-ai-agentic-architecture.md) — Agent orchestration, session concepts, cost tracking
- [32-ai-agentic-pydanticai.md](32-ai-agentic-pydanticai.md) — Coordinator `handle()`, entry points, streaming
- [33-ai-agent-first-infrastructure.md](33-ai-agent-first-infrastructure.md) — MCP/A2A (external agent interop, independent of messaging channels)
