...
_anthropic_breaker = aiobreaker.CircuitBreaker(
    fail_max=5,
    timeout_duration=30,
    listeners=[ResilienceLogger("llm_anthropic")],  # From doc 08
)
...
- [ ] Set up circuit breaker per provider (aiobreaker — per doc 16)
- [ ] Configure fallback chain in llm.yaml
- [ ] Verify resilience event logging (circuit breaker, retry, timeout — per doc 08)
