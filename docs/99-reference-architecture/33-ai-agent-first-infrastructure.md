# 33 — Agent-First Infrastructure (Optional Module)
...
- 1.1.0 (2026-02-24): Added Secure by Default principle (startup invariants, fail-closed enforcement), DM pairing protocol for messaging channels, channel-level rate limiting; references 27-opt-multi-channel-gateway.md
...
**Tools are thin adapters.** They call service methods. No business logic in MCP tool functions — the same rule as API endpoint handlers and PydanticAI tool functions (32-ai-agentic-pydanticai.md).
...
Extend the existing JWT authentication from **06-core-authentication.md** with MCP-specific audience validation:
...
This aligns with the AgentTask states in **31-ai-agentic-architecture.md** if both modules are adopted.
...
The agent inspects the plan, then confirms or abandons. This is the API equivalent of the HITL approval gates in **31-ai-agentic-architecture.md**.

### Structured Errors with Recovery Hints

Standard HTTP errors are opaque to agents. Extend the existing `ErrorResponse` from **10-core-error-codes.md** with agent-consumable recovery fields:
...
The existing authentication stack (**06-core-authentication.md**) covers human-era patterns: API keys, JWT, sessions. Agent identity requires three additional layers for autonomous, delegating, non-deterministic actors.
...
|----------|---------------|
| Single-service, no delegation | Existing JWT from 06-core-authentication.md is sufficient |
...
The **OpenTelemetry GenAI Semantic Conventions** (v1.37+, stable) provide standardized trace attributes for agent operations. These extend the existing structlog setup from **08-core-observability.md**.
...
Agent infrastructure logs are written to `logs/system.jsonl` with `source="agents"` per **08-core-observability.md**.
...
|-----------|-----------------|
| Agent Goal Hijacking | Guardrails decorator (32), input validation on MCP tools |
...
| Excessive Agency | UsageLimits on every agent run (32), cost-aware rate limiting (this document) |
...
When the platform is exposed through messaging channels (Telegram, Slack, Discord, WhatsApp) via **[27-opt-multi-channel-gateway.md](27-opt-multi-channel-gateway.md)**, unknown senders must be authenticated before their messages reach the agent coordinator. Three policies are available, configured per channel in `config/settings/gateway.yaml`:
...
For the full gateway security implementation, see **[27-opt-multi-channel-gateway.md](27-opt-multi-channel-gateway.md)**.
...
Rate limiting is enforced at the gateway level (**[27-opt-multi-channel-gateway.md](27-opt-multi-channel-gateway.md)**) using Redis for distributed state. When a limit is exceeded, the gateway responds with a channel-appropriate cooldown message and does not forward the message to the agent coordinator. Rate-limited messages do not consume agent budget.
...
- [31-ai-agentic-architecture.md](31-ai-agentic-architecture.md) — Internal agent orchestration (conceptual)
- [32-ai-agentic-pydanticai.md](32-ai-agentic-pydanticai.md) — Internal agent implementation (PydanticAI)
- [27-opt-multi-channel-gateway.md](27-opt-multi-channel-gateway.md) — Multi-channel delivery, DM pairing, channel rate limiting, session management
- [04-core-backend-architecture.md](04-core-backend-architecture.md) — FastAPI application where MCP/A2A is mounted
- [06-core-authentication.md](06-core-authentication.md) — Base authentication (extended by agent identity layers)
- [08-core-observability.md](08-core-observability.md) — Logging standards (extended by OTel GenAI conventions)
- [10-core-error-codes.md](10-core-error-codes.md) — Error code registry (extended by recovery hints)
- [30-ai-llm-integration.md](30-ai-llm-integration.md) — LLM provider interface
- [13-core-security-standards.md](13-core-security-standards.md) — Application security standards
