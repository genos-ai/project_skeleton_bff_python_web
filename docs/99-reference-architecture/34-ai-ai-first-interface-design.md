# 34 — AI-First Interface Design (Optional Module)
...
Extend the existing BFF architecture so that AI agents (Cursor, Claude Code, external orchestrators) are first-class consumers alongside CLI, TUI, and web clients. The service layer defined in `04-core-backend-architecture.md` already enforces business logic centralization — this module adds the interface adapters, discovery endpoints, content negotiation, and structured error extensions that make those services consumable by machines without duplicating code. All client types — human and AI — call the same service methods through thin adapters.

**Dependencies**: `04-core-backend-architecture.md` (service layer), `05-core-module-structure.md` (module boundaries), `10-core-error-codes.md` (error registry), `33-ai-agent-first-infrastructure.md` (MCP, A2A, agent identity). Composes with `31-ai-agentic-architecture.md` and `32-ai-agentic-pydanticai.md` for internal agents, and `27-opt-multi-channel-gateway.md` for channel delivery.
...
Already implemented per `04-core-backend-architecture.md`. Thin FastAPI endpoint handler:
...
### Adapter 2: MCP Tool (doc 33)

Already defined in `33-ai-agent-first-infrastructure.md`. The MCP tool function is a thin adapter:
...
### Adapter 3: PydanticAI Agent Tool (doc 32)
...
Implement the three discovery layers from `33-ai-agent-first-infrastructure.md`:
...
| `/.well-known/agent.json` | A2A Agent Card | External agents (A2A protocol) | New endpoint (doc 33) |
...
├── mcp/                              # EXISTS (doc 33) — use service factory
├── a2a/                              # EXISTS (doc 33) — use service factory
├── cli/
...
- Internal agent orchestration (covered by `31-ai-agentic-architecture.md` and `32-ai-agentic-pydanticai.md`)
- MCP server setup and A2A protocol integration (covered by `33-ai-agent-first-infrastructure.md`)
- Human-in-the-loop approval flows within agent workflows (covered by `31-ai-agentic-architecture.md` — durable execution section)
- Multi-channel gateway and session management (covered by `27-opt-multi-channel-gateway.md`)
- Frontend architecture (covered by `22-opt-frontend-architecture.md`)
- TUI architecture (covered by `26-opt-tui-architecture.md`)
